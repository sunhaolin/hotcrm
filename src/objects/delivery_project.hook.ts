 ...
 *
 * Step 27: when a delivery project is opened on (or moved to) a presales
 * project and no baseline was written, the Bizcase total (the four presales
 * rollups) becomes the default baseline; 「导入 Bizcase 预算」
 * (`cost_plan.actions.ts`) is the step that freezes it with cost plan v1. The
 * opportunity and account follow the presales project when they are blank.
 *
