...
// ledger is generated by `cost_line_decompose` for the delivery plans (one
// row per month); a Bizcase has no month rows, its lines carry their
// estimates; 江南 GMP stays a draft Bizcase.
...
export const industryEvents = defineSeed(Event, {
  ...
  records: [
    ...
    // 南方高速 is the quiet account (last touched 40 days ago, the lost bid): a
    // PLANNED re-engagement meeting keeps it on the calendar without moving
    // its clock — only a held event bubbles (`test/activity-seed-coverage`).
    { subject: '南方高速：联网收费改造预沟通', type: 'meeting', status: 'planned', start_datetime: celClock(-20, 10, 0), end_datetime: celClock(-20, 11, 30), duration_minutes: 90, all_day: false, related_to_type: 'crm_account', related_to_account: NF },
  ],
});
