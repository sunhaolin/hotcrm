...
export const costplan: Record<string, Partial<ObjectTranslationData>> = {
  ...
  crm_cost_plan: {
    ...
    fields: {
      ...
      phase: { label: '阶段', help: '保存时按所挂项目自动写入。Bizcase（售前）阶段的明细行不按月拆分，整段落为一行；交付阶段按月分解。', options: { bizcase: 'Bizcase（售前）', delivery: '交付' } },
      version_no: { label: '版本号', help: '同一项目下顺序递增；留空时保存自动编号。' },
      is_current: { label: '当前版本', help: '项目只读当前版本的金额；置为当前时其余版本自动作废。Bizcase 计划审批通过即自动成为当前版本。' },
      ...
      compare_plan: { label: '对比的当前版本', help: '提交审批时标记为当前版本的那个计划；本记录本身是当前版本时指向自己，差异全为 0。为空表示项目当时没有当前版本，下面的差异即全额新增。' },
      current_baseline_total: { label: '当前版本冻结基线' },
      delta_baseline_total: { label: '冻结基线差异' },
      current_planned_total: { label: '当前版本计划总额' },
      delta_planned_total: { label: '计划总额差异', help: '本次审批的计划总额减去提交时当前版本的计划总额；正数为增加，负数为核减；本记录即当前版本时为 0。' },
      delta_planned_pct: { label: '计划总额差异率 %', help: '差异 ÷ 提交时当前版本的计划总额 × 100。当前版本总额为 0 时读作 0。' },
    ...
    _sections: {
      basic: { label: '计划信息' },
      totals: { label: '计划金额' },
      comparison: { label: '与当前版本对比', description: '与「提交审批时」标记为当前版本的成本计划逐项对比，提交时取数、审批后保留为留痕。本记录本身就是当前版本时与自己对比，差异为 0。' },
      approval: { label: '审批' },
    },
  ...
  crm_cost_plan_month: {
    ...
    fields: {
      ...
      period_month: { label: '月份', help: '保存时归一到当月 1 日；Bizcase 计划的整段行记起始月份。' },
  ...
  crm_presales_project: {
    fields: {
      labor_cost: { label: '人工服务成本', help: '当前 Bizcase 成本计划的人工服务合计，不能手填；Bizcase 审批通过即成为当前版本。' },
      third_party_service_cost: { label: '第三方服务成本', help: '当前 Bizcase 成本计划的第三方服务合计，不能手填；Bizcase 审批通过即成为当前版本。' },
      procurement_cost: { label: '软硬件采购成本', help: '当前 Bizcase 成本计划的软硬件采购合计，不能手填；Bizcase 审批通过即成为当前版本。' },
      project_expense: { label: '项目费用', help: '当前 Bizcase 成本计划的项目费用合计，不能手填；Bizcase 审批通过即成为当前版本。' },
    },
  },
};
