...
const costLineFields = {
  ...
  estimate_amount: { label: 'Estimate Amount' },
  allocated_amount: { label: 'Allocated Amount' },
  notes: { label: 'Notes' },
};
...
export const psa: Record<string, ObjectTranslationData> = {
  ...
  crm_cost_plan: {
    ...
    fields: {
      ...
      planned_month_total: { label: 'Month Ledger Planned Total' },
      labor_month_total: { label: 'Month Ledger Labor Service Total' },
      service_month_total: { label: 'Month Ledger Third-party Service Total' },
      procurement_month_total: { label: 'Month Ledger Hardware/Software Procurement Total' },
      expense_month_total: { label: 'Month Ledger Project Expense Total' },
      travel_month_total: { label: 'Month Ledger Of Which Travel' },
      labor_line_total: { label: 'Line Estimate Labor Service Total' },
      service_line_total: { label: 'Line Estimate Third-party Service Total' },
      procurement_line_total: { label: 'Line Estimate Hardware/Software Procurement Total' },
      expense_line_total: { label: 'Line Estimate Project Expense Total' },
      travel_line_total: { label: 'Line Estimate Of Which Travel' },
      planned_total: { label: 'Planned Total', help: 'The sum of the four line estimates on a Bizcase; the sum of the month ledger on a delivery plan.' },
      ...
      compare_plan: { label: 'Compared Current Version', help: 'The plan marked Current Version when this one was submitted; it points at this record itself when this record is already the current version, so every difference reads 0. Empty means the project had no current version then and every difference below is the full increase.' },
      ...
      delta_planned_total: { label: 'Planned Total Difference', help: 'The planned total under approval minus the planned total of the version in force at submit time; positive is an increase, negative a reduction, and 0 when this record is itself the current version.' },
      delta_planned_pct: { label: 'Planned Total Difference %', help: 'Difference ÷ the planned total of the version in force at submit time × 100. Reads 0 while that total is 0.' },
    ...
    _sections: {
      ...
      comparison: { label: 'Comparison with the Current Version', description: 'Item by item against the plan marked Current Version AT SUBMIT TIME, snapshotted on submission and kept afterwards as the approval record. A record that is itself the current version compares with itself, so every difference is 0.' },
      approval: { label: 'Approval' },
    },
    _actions: {
