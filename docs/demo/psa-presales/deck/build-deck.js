...
// 1 · cover
{
  ...
  notes(s, '各位好。今天这场演示只讲一件事：贵司给我们的那张 40 步、9 个环节的业务流程表，怎样在一个平台、一套数据里从头跑到尾。表里把它分成了 CRM 和项管平台两个系统，今天大家会看到，它们其实是同一个系统里的两组业务对象。成本计划按贵司的要求做成了带版本、四类明细、按月分解的形态，审批的发起是记录上的一个按钮，这两处后面会重点看。下面 40 分钟我按贵司流程表的顺序走，每一页右上角都标着对应的步骤号。');
}
...
// 2 · headline numbers
{
  ...
  notes(s, '先给结论。40 步都在同一个平台、同一套数据里跑起来了：36 步基本实现，主体功能今天可以现场演示，细节还在打磨，欢迎各位边看边提；4 步部分实现——商机状态变更还不单独审批，售前审批今天是一级、生产上走五级，成本计划新版本的预算调整通过后「当前预算」的联动还没有完成，加班工时还是手填。为此新建了 20 个业务对象、10 条审批流、4 处系统拦截加 2 处冻结，全部跑在同一个审批中心里。怎么做到的：不做两套系统再做接口，而是把项目管理长在 CRM 的对象上，商机编码是两侧唯一的锚点；业务规则落在系统里而不是制度里；汇总和公式都是实时算的。');
}
...
// 5 · three layers
{
  ...
  notes(s, '三层。最上面是平台自带的能力：审批中心、附件、活动时间线、汇总字段、公式字段、权限、仪表板、导入导出、记录锁定——这一层零开发。中间是 CRM 的标准对象加字段：客户、联系人、线索、商机、销售合同这些。底下是本次为贵司流程新建的 17 个项目对象——包括成本计划、四类成本明细行和月度分解行——再加费率卡、差旅标准、签约主体三项主数据。串起三层的是商机编码：售前项目必须挂在已立项的商机上，交付项目必须挂在已审批的售前项目上，成本、工时、差旅、开票、收款都挂在交付项目下。');
}
...
// 6 · coverage table
{
  ...
  notes(s, '这张表逐环节说清做到了什么，各位可以对照自己那张表看。绿色是基本实现——主体功能今天可以演示，细节还在打磨，欢迎各位边看边提；橙色是部分实现，四处：商机状态变更目前不单独审批；售前审批今天是一级，生产上按贵司组织层级走五级；成本计划的新版本预算调整审批通过后，新版本已经生效、计划总额也跟着变了，但交付项目上「当前预算」这一个数的联动还没有完成；成本执行里请假同步已经做了，加班同步还是手填。项目报表环节的开票、收款是手工录入，对接财务系统放在后续阶段。接下来按环节一个个看。');
}
...
// 10 · 客户管理
{
  ...
  notes(s, '进入第一个环节，步骤 1 到 5。此时切到系统，打开华信科技这家客户。客户编码自动分配；基本信息里有贵司要的客户分类、客户简称、统一社会信用代码。往下是业务信息分组：当前主要服务商、本年度 IT 采购预算、付款周期、美国 EAR 管制清单、战略合作伙伴。联系人多了性别、角色、对我司态度、与销售关系强度四个字段。附件页签直接拖资料进来。客户点「发起审批」后记录锁定，通过后才正式生效——审批怎么发起，后面专门有一页讲。');
}
...
// 14 · the submit button
{
  ...
  notes(s, '审批的发起我们做成了一个按钮，而不是编辑记录去改状态。每个可审批对象的记录页顶部都有「发起审批」，商机上叫「发起立项审批」；按钮只在草稿或已驳回时出现，点了弹确认框，确认后记录立刻锁定、显示审批中。审批状态和审批通过时间是只读字段——表单里看得到、改不了，只由按钮和审批流来写。驳回后按钮重新出现，改完再发起。');
}
...
// 15 · approval swimlane (native)
{
  ...
  notes(s, '这页把审批画成三条泳道：发起人、系统、审批人。发起人只做三件事——打开记录、点「发起审批」、确认；商机上这个按钮叫「发起立项审批」。系统把状态置为已提交、锁定记录、生成审批请求并路由到审批人。审批人在待我审批里勾选、通过 N 条或拒绝。系统随后回写审批状态与审批日期、解锁记录、通知负责人；驳回的记录可以修改后再次发起。要强调两条原则：第一，审批状态字段是只读的，表单和接口都改不了，只有按钮和审批流能改它，所以状态始终等于审批结果，不会出现"手工改成已通过"的漏洞；第二，十条流程共用一个审批中心、一套手势，审批人不需要学十种操作。');
}
...
// 16 · 商机跟进与铁三角
{
  ...
  notes(s, '步骤 12 到 14。跟进不靠记忆：阶段条直接推进阶段，每次变更自动写进活动时间线；待办任务和活动挂在商机上，右侧任务卡常驻。铁三角——客户经理、解决方案经理、交付经理——三个角色就地调整。赢单、丢单必须填原因。北辰 MES 二期是已成交商机，赢单原因是客户关系。步骤 14 的状态变更审批目前没有做：赢单、弃单、调整铁三角还不单独走审批，这一步我们如实标为未实现。生产形态下铁三角会做成商机团队对象加记录级共享，团队成员自动获得可见性。');
}

// 17 · section — 项管平台
{
  const s = section('02', '第二部分 · 项管平台', '售前 · 交付 · 成本计划 · 成本执行 · 报表', '贵司需求表步骤 15–40：立项、成本计划、成本执行与管控、项目报表', [4, 5, 6, 7, 8]);
  notes(s, '第二部分是项管平台侧的 26 步，也是本次新增的部分：售前立项、交付立项、成本计划、成本执行、项目报表。每一步都读 CRM 的商机，这是把它们放在同一个平台上的原因。这一部分的重点是成本计划：带版本、四类明细、按月分解。');
}
...
// 20 · cost plan: three layers
{
  ...
  notes(s, '步骤 27 到 31，成本计划分三层。最上面是成本计划本身，它是"一个版本"：挂在售前项目上就是 Bizcase，挂在交付项目上就是交付计划；有版本号、当前版本、冻结基线；计划总额和人工、第三方、采购、费用四类合计全是汇总出来的，没有一个是手填的。中间是四类明细行，贵司需求表里四个配置界面对应四个对象，因为四类成本的算法和单价来源各不相同，下一页展开。最底下是月度分解行：保存一条明细行，系统按起止月份自动生成一行一个月的台账，右边截图就是华信一期交付计划的台账——成本类别、月份、金额逐行可查；改过某个月，它就标成手工调整，重新分解也不会覆盖。所有合计、售前项目的四项成本、交付项目的计划总额、项目成本仪表板，读的都是这张台账。');
}
...
// 33 · simplifications
{
  ...
  notes(s, '把简化说在前面。售前审批今天一级，生产五级串联、按组织层级自动路由；商机状态变更审批未做；成本计划版本化的预算调整，审批通过后新版本已经生效、计划总额也更新了，「当前预算」这一个数的联动还没有完成；铁三角做成团队对象加共享；请假同步今天按工作日乘 8 小时算，加班手填，后续对接考勤；报销单据、开票收款后续对接费控和财务。审批发起已经改成按钮加只读状态。演示数据是一家软件公司的样子，上线时换成贵司真实的客户、项目、费率卡和差旅标准。');
}
...
// 34 · roadmap
{
  ...
  notes(s, '三个阶段。阶段一试点上线，就是今天演示的范围，先和贵司确认五个口径：跟进记录形态、开票回款归属、线索形态、EAR 判定方式、立项审批与金额审批的关系；我们这边完成预算调整的当前预算联动，然后按组织和岗位配置审批人、权限、费率卡、差旅标准、签约主体，选一个事业部一批真实项目试运行——先把 Bizcase 编进成本计划，再导入交付项目。阶段二做规则和协同：五级审批、状态变更审批、铁三角共享、预算多级审批。阶段三打通考勤、费控、财务三个外部系统。');
}
...
// 35 · closing
{
  ...
  notes(s, '完成一句话：贵司 40 步流程里，客户、线索、商机、审批中心、附件、跟进、赢丢单是现成的 CRM 能力；售前立项、交付立项、成本计划、工时与差旅、财务与报表这 26 步，是在同一个平台、同一套数据里长出来的；成本计划按贵司要求做成了带版本、四类明细、按月分解的形态——这也是它能成为软件行业项目型销售标准模板的原因。下一步三个动作：确认五个口径，划定试点范围，我们给出排期。谢谢大家，欢迎提问。');
}
