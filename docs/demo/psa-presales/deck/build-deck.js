...
// 13 · approval center
{
  ...
  notes(s, '贵司的流程里审批反复出现，我们把它做成了一套动作。发起：记录页顶部点「发起审批」，确认。处理：待我审批，勾选，通过 N 条，确认；通过后系统自动回写状态和审批时间，通知负责人。转签、退回修改、退回补充、催办、撤回都在同一个面板里。今天一共十条流程——客户、线索、商机立项、售前立项、交付立项、成本计划、预算调整、工时、出差、请假——全在这一个收件箱里；左边这张图就是成本计划审批和预算调整审批并排等着处理，每一行都写着记录和金额，审批人不用点开就知道在审什么。生产上按贵司组织层级扩成多级就行。那发起审批的那个按钮，为什么值得单独讲一页？——');
}
...
// 14 · the submit button
{
  ...
  notes(s, '因为它决定了审批状态可不可信。我们把审批的发起做成一个按钮，而不是让人编辑记录去改状态。每个可审批对象的记录页顶部都有「发起审批」，商机上叫「发起立项审批」；按钮只在草稿或已驳回时出现，点了弹确认框，确认后记录立刻锁定、显示审批中。审批状态和审批通过时间是只读字段——表单里看得到、改不了，只有按钮和审批流能写它。驳回之后按钮重新出现，改完再发一次。把这个过程画成图，就是下一页——');
}
...
// 15 · approval swimlane (native)
{
  ...
  para(s, '两条设计原则：① 审批状态字段只读——表单改不了，接口也写不进，只有这个按钮和审批流能改它，所以状态始终等于审批结果。② 十条流程共用同一个审批中心和同一套手势：发起 = 按钮 + 确认；处理 = 待我审批 → 勾选 → 通过 N 条。审批期间记录锁定，通过后回写审批日期，驳回后可修改再提。', M + 0.25, 5.52, CW - 0.5, 1.2, { size: 12, color: C.ink });
  notes(s, '三条泳道：发起人、系统、审批人。发起人只做三件事——打开记录、点「发起审批」、确认。系统接手：状态置为已提交、锁定记录、生成审批请求、路由给审批人。审批人在待我审批里勾选，通过或者拒绝。系统再回写状态和日期、解锁记录、通知负责人；被驳回的记录改完可以再发。我想强调两条原则：第一，审批状态字段是只读的，表单改不了，接口也写不进，所以状态始终等于审批结果，不会出现"手工改成已通过"的漏洞；第二，十条流程共用一个审批中心、一套手势，审批人不用学十种操作。审批说完，回到商机本身，看看立项之后怎么跟——');
}
...
// 16 · 商机跟进与铁三角
{
  ...
  bullets(s, [
    ...
    { text: '步骤 14 商机状态变更审批：未实现——赢单 / 弃单 / 铁三角调整目前不单独走审批', color: C.amberDeep, bold: true },
    '生产形态：铁三角做成商机团队对象 + 记录级共享，团队成员自动获得可见性',
  ], M, 2.0, 4.6, 4.7, { fontSize: 12, gap: 7 });
  shot(s, 'S13-铁三角', 5.5, 1.5, 7.23, '商机详情：铁三角与赢单 / 输单分组');
  notes(s, '步骤 12 到 14。跟进不靠记忆：阶段条直接推进阶段，每次变更自动写进活动时间线；待办任务和活动挂在商机上，右侧任务卡常驻。铁三角——客户经理、解决方案经理、交付经理——三个角色就地调整。赢单、丢单必须填原因，北辰 MES 二期就是已成交的商机，赢单原因是客户关系。这里要如实说一句：步骤 14 的状态变更审批目前没有做，赢单、弃单、调整铁三角还不单独走审批，我们标的是未实现。生产形态下铁三角会做成商机团队对象加记录级共享。CRM 这一段到此，商机已经立项、跟进也在走，接下来就该进项管平台了——');
}
