...
| Cost Plan | Private | A plan version is its cost manager's working record; the sales manager and the administrator hold *View All* |
| Labor Cost Line | Controlled by Parent (Cost Plan) | A labor line is part of one plan version, and follows the plan's sharing |
| Service Cost Line | Controlled by Parent (Cost Plan) | A third-party service line is part of one plan version, and follows the plan's sharing |
| Procurement Cost Line | Controlled by Parent (Cost Plan) | A procurement line is part of one plan version, and follows the plan's sharing |
| Expense Cost Line | Controlled by Parent (Cost Plan) | An expense line is part of one plan version, and follows the plan's sharing |
| Cost Plan Month | Controlled by Parent (Cost Plan) | The month ledger is derived from the plan's lines, and follows the plan |
| Travel Standard | Public Read/Write | Master data every travel expense line prices from; maintained by the PMO |
...
For the seventeen parent-derived objects above, there is nothing to configure per user: access is *computed*.
...
A **position** is a flat group used to distribute access. HotCRM ships thirteen:
...
`system_admin` is the one position no sharing rule names. It carries the **permission set** of the same name instead: a position and a permission set sharing a name are bound to each other, which is how the administrator profile — including its export rights — reaches an actual person. Assign it exactly as sparingly as that implies. (Under `HOTCRM_COMPOSITION=saas` this row reads `tenant_admin`, matching the set that build registers in its place.)
